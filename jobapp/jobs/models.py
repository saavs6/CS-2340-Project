...
class Job(models.Model):
    ...
    moderation_status = models.CharField(max_length=20, choices=MODERATION_STATUS_CHOICES, default='approved', help_text='Moderation status for admin review')
...
class JobRecommendation(models.Model):
    """Recommendation of a job for an applicant generated by the recommendation engine"""

    applicant = models.ForeignKey('auth.User', on_delete=models.CASCADE, related_name='job_recommendations')
    ...
    def __str__(self):
        return f"Recommendation: {self.job.title} -> {self.applicant.username} ({self.match_score:.1f})"